/* The difference between the pointer implementation of link list and array implementation of linked list:
1. The NULL is represented by -1;
2. Limitede size. (in the following case it is 100 nodes at max). But we can reuse the nodes that are to be deleted by again linking it back to the list.
*/
...
Node AvailArray[100]; //array taht will act as nodes of a liked list
int head = -1;
int avail = 0;
...
{
    int NodeInexToBeReturned = avail;
    avail = AvailArray[avail].next;
    return NodeInexToBeReturned;
}

void freeNode(int nodeToBeDeleted) // This function when called will delete the node  with the inex presented as an argument, and will put back that put back that node into the array
{
...
int main()
{
    ...
    for(;;)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            cout << "Ther linked list contains the following element in order" << endl;
            display();
            break;
        case 4:
            exit(0);
