...
void (merge(int arr[], int l, int m, int r))
{
    ...
    //after comparing, we merge those two array inmlarger sub array
    while (i < len1 && j < len2)
    {
        if (left[i] <= rigth[j])
        {
...
void timSort(int arr[], int n)
{
    ...
    // start merging from size RUN (or 32). It will merge to from size 64, then 128, 256 and so on ...
...
//Drivers program to test above function
int main()
{
