    ...
    do 
    {
        ...
        else 
        {
            ...
            if (!rehashing) cout << "Spot taken, looking at next (next index:"
                << " " << static_cast<int>(fabs((hash +) (i * otherHashFxn(key)))) % totalSize << ")" << endl;
            i++;
        }
        if (i == totalSize * 100)
        {
...
void rehash()
{
    // Necessary so wall of add info isn't printed at once
    rehashing = true;
    int oldSize = totalSize;
    Entry* oldTable = table;
    // Really this shold use the next prime number greater than totalSize * 2
...
// Removes key. Leaves tombstone upnon removal.
void remove(int key)
{
