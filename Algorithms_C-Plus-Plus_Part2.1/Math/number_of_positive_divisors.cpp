...
int number_of_positive_divisors(int n)
{
    ...
    for (int i=2; i*i <= n; i++)
    {
        ...
        if (prime_cout != 0)
            prime_exponent_count.push_back(prime_cout);
    }
    if (n> 1) 
    ...
    {
        divisors_cout = divisors_cout * (prime_exponent_count[i]+1);
    }

    return divisors_cout;
}
...
int main()
{
    ...
    if (n < 0)
        n = -n;
    if (n == 0)
        std::cout << "All non-zero nubers are divisors of 0 !" << std::endl;
    else 
    {
