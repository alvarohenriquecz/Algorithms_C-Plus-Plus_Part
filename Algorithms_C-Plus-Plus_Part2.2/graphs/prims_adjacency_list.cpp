...
int main()
{
    ...
    cout << "Enter the vertices and their edges weigths in the following format :" << endl;
    cout << "Vertex1 Vertex2 Weigth" << endl;
    ...
    while(!Q.empty())
    {
        p = Q.top();
        selected = p.second;
        // Edges with minimum weight is selected
        Q.pop();

        for(int i = 0; i < adj[selected].size(); i++)
        {
            int next = adj[selected][i].second;
            // This variable stores index of adjacent vertices to 'selected'
            int weigth = adj[selected][i].first;

            if(!included[next] && keys[next] > weigth)
            {
                keys[next] = weigth;
