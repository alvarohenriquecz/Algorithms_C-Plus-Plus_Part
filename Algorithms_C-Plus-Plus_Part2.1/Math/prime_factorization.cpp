...
// Declarating Variables for maintaing prime numbers and to check whther a number is prime or not
bool isprime[1000006];
vector<int> prime_numbers;
vector<pair<int, int>> factor;

// Calculating prime number upto a given range
void SieveOfEratorthemes(int N)
{
...
// Primefactorization of a number
void prime_factorization(int num)
{
    ...
    for (int i = 0; prime_numbers[i] <= num; i++)
    {
        ...
        if (count)
            factor.push_back(make_pair(prime_numbers[i], count));       
    }
}
